...
class cover_parser:
    def __init__(self, image_folder_or_file, data_file, **kwargs):
        ...
        self.image_data, self.tag_data, self.ids = self.get_tags_features(data_file, image_dict)
    ...
    def load_image(self, image_folder_or_file):
        try:
            ...
            image_list = list(filter(lambda x: x[-4:] == 'jpeg', os.listdir(image_folder_or_file)))
            image_dict = {}
    ...
    def get_tags_features(self, data_file, image_dict):
        ...
        for id, tag in zip(ids, binned_tags):
            try:
                ...
                continue
        self.logger.info('Image data length: %d, image shape %s' % (len(image_dict), str(image_data[-1].shape)))
    ...
    def run_model(self, mode="train", trainning_size=0.8, num_epoch=20, batch_size=100, lr=1e-4, **kwargs):
        ...
        if mode == "train":
            ...
            for epoch in range(num_epoch):
                ...
                if epoch % kwargs['print_interval'] == 0:
                    with torch.no_grad():
                        cpc_pred_train, train_loss = self.model(train_data.to(device=self.device, dtype=torch.float32), train_tags.to(self.device))
                        cpc_pred_test, test_loss = self.model(test_data.to(device=self.device, dtype=torch.float32), test_tags.to(self.device))
                        cpc_pred_worst = (
                ...
                for i in range(n_batch):
                    ...
                    data = torch.from_numpy(input_data[data_inds]).to(device=self.device, dtype=torch.float32)
...
def main():
    video_cover_parser = cover_parser('../Data/image_data.npy', '../Data/kuaishou_data_0421.csv', log_level=logging.INFO)
    video_cover_parser.run_model(num_epoch=100, batch_size=50, lr=1e-3, print_interval=1)
